...
export default function DashboardPage() {
  ...
  const derived = useMemo(() => {
    ...
    // Allowance uses all-time books. Scoped filters only apply to charts/revenue/etc.;
    // otherwise "this month" would drop most income and freeze allowance at ₹0.
    const myBalAll = calcFounderBalance(user.id, data);
    const partnerBalAll = partnerUser ? calcFounderBalance(partnerUser.id, data) : myBalAll;

    const bankMin = 50000;
    const companyBank = myBalAll.balance + partnerBalAll.balance;
    const currentPool = Math.max(0, companyBank - bankMin);

    const myW = myBalAll.totalPersonalWithdrawals;
    const partnerW = partnerBalAll.totalPersonalWithdrawals;
    ...
    const allocatedPartner = allocatedMy;

    const myRemainingAllowance = allocatedMy - myW;
    const partnerRemainingAllowance = allocatedPartner - partnerW;
        ...
        <div style={{ padding: '0 16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
            {[
              ...
              { label: 'Allowance', val: formatCurrency2(derived.myAllowance), color: derived.myAllowance >= 0 ? '#00ff41' : '#ff0033', icon: Wallet },
          ...
          >
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 18, padding: 16, marginBottom: 16, border: '1px solid rgba(255,255,255,0.06)' }}>
              {[
                ...
                { label: 'Allowance',      val: formatCurrency2(derived.myAllowance),                         color: derived.myAllowance >= 0 ? 'rgba(255,255,255,0.5)' : '#ff0033' },
              ].map(({ label, val, color }) => (
